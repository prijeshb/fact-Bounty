...
class LankaNewsWebSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for news in response.css('div.itemContainer'):
            ...
            image_url = news.css('div.catItemImageBlock a img attr(src)').extract_first()
            date_time = news.css('aside.article-aside dl.article-info dd.create ::text').extract_first()
        ...
        # goto next page
        next_url = response.css('div.k2Pagination ul.pagination li a[title*=Next] ::attr(href)').extract_first()
