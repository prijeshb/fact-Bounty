...
class BeautySpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for news in response.css('div.small-12.medium-4.large-4.columns'):
            ...
            item['news_headline'] = news.css('header.post-title.entry-header h5 ::text').extract_first()
            item['datetime'] = news.css('aside.post-author.cf time ::text').extract_first()
            news_url = news.css('header.post-title.entry-header h5 a ::attr(href)').extract_first()
            item['link'] = news_url
    ...
    def parse_1(self, response):
        data = response.css('div.post-content.entry-content.cf p ::text').extract()
        data = data[:-1]
