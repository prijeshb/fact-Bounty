...
class BizadaDeranaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for news in response.css('div.col-lg-12.business-summary'):
            news_headline = news.css('div.summary-news h3 a ::text').extract_first()
            news_url = news.css('div.summary-news h3 a ::attr(href)').extract_first()
            image_url = news.css('div.summary-news div.col-lg-4.thumb-image a img ::attr(src)').extract_first()
            date_time = news.css(
        ...
        next_link = response.css('ul.pager a[title*=next] ::attr(href)').extract_first()
