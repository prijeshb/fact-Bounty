...
class adaDeranaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for news in response.css('div.sports'):
            news_headline = news.css('div.story-text h4 a ::text').extract_first()
            news_url = news.css('div.story-text h4 a ::attr(href)').extract_first()
            image_url = news.css('div.col-xs-3.thumb-image a img ::attr(src)').extract_first()
            date_time = news.css('div.col-xs-12.comments span ::text').extract_first()
