...
class RAWSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for news in response.css('div.row.cat-zero'):
            news_headline = news.css('div.col-md-7 a h1.cat-header ::text').extract_first()
            news_url = news.css('div.col-md-7 a ::attr(href)').extract_first()
            image_url = news.css('div.col-md-5 a img.img-responsive ::attr(src)').extract_first()
            date_time = news.css('div.col-md-7 p.cat-other span ::text').extract_first()
