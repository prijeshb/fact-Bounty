...
class LankadeepaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for news in response.css('div.row'):
            news_url = news.css('div.simple-thumb a ::attr(href)').extract_first()
            title = news.css('div.simple-thumb a.news-title ::text').extract_first()
            timedate = news.css('div.simple-thumb p.timeandauthor small ::text').extract_first()
        ...
        next_url = response.css('a.last.page-numbers ::attr(href)').extract_first()
