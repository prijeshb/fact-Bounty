...
class reporterSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for news in response.css('article.post'):
            news_headline = news.css('header.entry-header h1.entry-title a ::text').extract_first()
            news_url = news.css('header.entry-header h3.entry-title a ::attr(href)').extract_first()
            image_url = news.css('div.post-image a img ::attr(src)').extract_first()
            date_time = news.css('div.entry-meta span.posted-on time.entry-date ::text').extract_first()
