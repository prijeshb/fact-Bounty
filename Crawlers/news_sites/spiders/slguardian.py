...
class slgurdianSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for news in response.css('article.widget-entry.entry-large.col-sm-6'):
            news_headline = news.css('header.entry-header h3.entry-title a ::text').extract_first()
            news_url = news.css('header.entry-header h3.entry-title a ::attr(href)').extract_first()
            image_url = news.css('div.entry-thumbnail a ::attr(src)').extract_first()
            date_time = news.css(
        ...
        # goto next page
        next_url = response.css('div.nav-links a.next.page-numbers ::attr(href)').extract_first()
