...
class CarSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for news in response.css('div.ia-card__content'):
            ...
            item['news_headline'] = news.css('a.ia-card__title ::text').extract_first()
            item['telephone'] = news.css('strong a  ::text').extract_first().split(':')[1].rsplit()[0]
            item['link'] = news.css('a.ia-card__title ::attr(href)').extract_first()
            item['newsInDetails'] = news.css('p.ia-car-summary  ::text').extract_first()
            items.append(item)
        yield {"data": items}
